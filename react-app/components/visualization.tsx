...
const NodeComponent = ({ node, openModal }) => {
  const width = 250
  const height = 80
  const x = node.x - width / 2
  const y = node.y - height / 2
  return (
    <g style={{ cursor: 'pointer' }} onClick={openModal}>
 ...
 */
export function SchemaVisualizer({ width, height, nodes, links }) {
    ...
    return (
      <>
        <svg id="vizgraph" width={width} height={height} style={{ backgroundColor: '#D2D2D2'}}>
            ...
            {
                simData.nodes.map((node, index) => <NodeComponent node={node} key={index} openModal={toggleModal} />)
            }
        </svg>
        <Modal isOpen={isModalOpen} onRequestClose={toggleModal} contentLabel="details modal" >
          <h2>Node details here!</h2>
